...
(function( window, undefined ) {
    ...
    var Keycloak = function (config) {
      ...
      kc.loginForm = function (options) {
        var params = `grant_type=password&username=${options.username}&password=${options.password}&client_id=${kc.clientId}&client_secret=${kc.clientSecret}&scope=openid`;
        ...
        req.onreadystatechange = function () {
          if (req.readyState == 4) {
            if (req.status == 200) {
              ...
              localStorage.setItem('keyTok',req.responseText);
              var tokenResponse = JSON.parse(req.responseText);
              setToken(tokenResponse['access_token'], tokenResponse['refresh_token'], tokenResponse['id_token'], timeLocal);
              console.info('[KEYCLOAK] Login');
      ...
      kc.createLogoutUrl = function(options) {
        ...
        kc.isTokenExpiredWithToken = function(){
         var responseText = localStorage.getItem('keyTok');
          var tokenResponse = JSON.parse(responseText);
          var timeLocal = new Date().getTime();
          timeLocal = (timeLocal + new Date().getTime()) / 2;
          setToken(tokenResponse['access_token'], tokenResponse['refresh_token'], tokenResponse['id_token'], timeLocal);
